#!/bin/bash
# Finetune script for LLaVA-LLaDA-BD3LM model
# Model type: llava_llada_bd3lm (External ViT + LLaDA LLM + BD3-LM)
...
# Model paths - External ViT + LLaDA LLM
...
OUTPUT_DIR="./outputs/llava_llada_bd3lm_finetune"
...
num_node=$1
gpu_num=$2
custom_run_name=${3:-"llava_llada_bd3lm_finetune"}
...
echo "=========================================="
echo "LLaVA-LLaDA-BD3LM Finetune (LLaDA + BD3-LM)"
echo "=========================================="
...
torchrun --nproc_per_node=${gpu_num} --nnodes=${num_node} --master_addr=${MASTER_ADDR} --master_port ${MASTER_PORT} --node_rank=${RANK} \
    ...
    --force_model_type "llava_llada_bd3lm"
